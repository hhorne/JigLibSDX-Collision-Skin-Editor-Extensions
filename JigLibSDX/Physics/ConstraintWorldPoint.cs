...
namespace JigLibSDX.Physics
{
    ...
    public class ConstraintWorldPoint : Constraint
    {
        ...
        public override bool Apply(float dt)
        {
            ...
            #region REFERENCE: Vector3 worldPos = body.Position + Vector3.TransformCoordinate(pointOnBody, body.Orientation);
            Vector3 worldPos;
            Vector3.TransformCoordinate(ref pointOnBody, ref body.transform.Orientation, out worldPos);
            ...
            if (body.CollisionSkin != null)
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    if (collInfo.SkinInfo.Skin1.Owner == null)
                    {
                        ...
                        float dot;
                        dot = Vector3.Dot(desiredVel, dir);
                        #endregion
            ...
            #region REFERENCE: float denominator = body.InvMass + Vector3.Dot(N, Vector3.Cross(Vector3.TransformCoordinate(Vector3.Cross(R, N), body.WorldInvInertia), R));
            Vector3 v1; float f1;
            Vector3.Cross(ref R, ref N, out v1);
            Vector3.TransformCoordinate(ref v1, ref body.worldInvInertia, out v1);

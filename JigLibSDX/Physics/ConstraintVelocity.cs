    ...
    public class ConstraintVelocity : Constraint
    {
        ...
        public override bool Apply(float dt)
        {
            ...
            if (frame == ReferenceFrame.Body) // transfrom velocity to the body frame
            {
                if (doVel)
                {
                    #region REFERENCE: Vector3 velBodyFrame = Vector3.TransformCoordinate(vel, body.Orientation);
                    Vector3 velBodyFrame;
                    Vector3.TransformCoordinate(ref vel, ref body.transform.Orientation, out velBodyFrame);
                    #endregion
                ...
                if (doAngVel)
                {
                    #region REFERENCE: Vector3 angVelBodyFrame = Vector3.TransformCoordinate(angVel, body.Orientation);
                    Vector3 angVelBodyFrame;
                    Vector3.TransformCoordinate(ref angVel, ref body.transform.Orientation, out angVelBodyFrame);
                    #endregion

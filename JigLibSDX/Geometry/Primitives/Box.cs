...
namespace JigLibSDX.Geometry
{
    ...
    public class Box : Primitive
    {
        ...
        /// </summary>
        public Box()
            : base((int)PrimitiveType.Box)
        {
            this.transform = Transform.Identity;
            this.sideLengths = new Vector3(1f);
        }
        ...
        public Vector3 GetCentre()
        {
            ...
            Vector3.TransformCoordinate(ref result, ref transform.Orientation, out result);
        ...
        public void GetCentre(out Vector3 centre)
        {
            ...
            Vector3.TransformCoordinate(ref centre, ref transform.Orientation, out centre);
        ...
        {
            transform.Position -= Vector3.TransformCoordinate(amount, transform.Orientation);
        ...
        {
            closestBoxPoint = Vector3.TransformCoordinate(point - transform.Position, Matrix.Transpose(transform.Orientation));
            ...
            Vector3.TransformCoordinate(ref closestBoxPoint, ref transform.Orientation, out closestBoxPoint);
        ...
        public void GetSpan(out float min, out float max, Vector3 axis)
        {
            ...
            MatrixHelper.GetRight(ref orientation, out right);     //Vector3 right = transform.Orientation.Right;
            MatrixHelper.GetUp(ref orientation, out up);           //Vector3 up = transform.Orientation.Up;
            MatrixHelper.GetBackward(ref orientation, out back);   //Vector3 back = transform.Orientation.Backward;

            s = Vector3.Dot(axis, right);
            u = Vector3.Dot(axis, up);
            d = Vector3.Dot(axis, back);
            ...
            float r = s + u + d;
            GetCentre(out right);
            float p = Vector3.Dot(right, axis);
            min = p - r;
            max = p + r;
        }
        ...
        public void GetCornerPoints(out Vector3[] pts)
        {
            ...
            pts[(int)BoxPointIndex.FRD] = transform.Position + sideLengths.X * MatrixHelper.GetRight(ref orientation);             //transform.Orientation.Right;
            pts[(int)BoxPointIndex.BLD] = transform.Position + sideLengths.Y * MatrixHelper.GetUp(ref orientation);                //transform.Orientation.Up;
            pts[(int)BoxPointIndex.BRU] = transform.Position + sideLengths.Z * MatrixHelper.GetBackward(ref orientation);          //transform.Orientation.Backward;
            pts[(int)BoxPointIndex.FLD] = pts[(int)BoxPointIndex.BLD] + sideLengths.X * MatrixHelper.GetRight(ref orientation);    //transform.Orientation.Right;
            pts[(int)BoxPointIndex.BLU] = pts[(int)BoxPointIndex.BRU] + sideLengths.Y * MatrixHelper.GetUp(ref orientation);       //transform.Orientation.Up;
            pts[(int)BoxPointIndex.FRU] = pts[(int)BoxPointIndex.FRD] + sideLengths.Z * MatrixHelper.GetBackward(ref orientation); //transform.Orientation.Backward;
            pts[(int)BoxPointIndex.FLU] = pts[(int)BoxPointIndex.FLD] + sideLengths.Z * MatrixHelper.GetBackward(ref orientation); //transform.Orientation.Backward;
        }

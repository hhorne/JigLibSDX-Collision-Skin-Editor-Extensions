    ...
    public class Octree
    {
        ...
        public unsafe int GetTrianglesIntersectingtAABox(int* triangles, int maxTriangles, ref BoundingBox testBox)
        {
            ...
            while (curStackIndex < endStackIndex)
            {
                ...
                curStackIndex++;
                if (SlimDX.BoundingBox.Contains(nodes[nodeIndex].box, testBox) != ContainmentType.Disjoint)
                {
                    for (int i = 0; i < nodes[nodeIndex].triIndices.Length; ++i)
                    {
                        if (SlimDX.BoundingBox.Contains(triBoxes[nodes[nodeIndex].triIndices[i]], testBox) != ContainmentType.Disjoint)
                        {

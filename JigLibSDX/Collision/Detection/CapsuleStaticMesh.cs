            ...
            unsafe
            {
                ...
                fixed (SmallCollPointInfo* collPts = collPtArray)
                {
                    ...
                    fixed (int* potentialTriangles = potTriArray)
                    {
                        ...
                        Vector3 meshSpaceCapsuleStart = Vector3.TransformCoordinate(capsuleStart, meshInvTransform);
                        Vector3 meshSpaceCapsuleEnd = Vector3.TransformCoordinate(capsuleEnd, meshInvTransform);
                        ...
                        for (int iTriangle = 0; iTriangle < numTriangles; ++iTriangle)
                        {
                            ...
                            float distToStart = SlimDX.Plane.DotCoordinate(meshTriangle.Plane, meshSpaceCapsuleStart);
                            float distToEnd = SlimDX.Plane.DotCoordinate(meshTriangle.Plane, meshSpaceCapsuleEnd);
            ...
            TriangleMesh mesh, CollDetectInfo info, float collTolerance, CollisionFunctor collisionFunctor)
        {
            ...
            else
            {
                ...
                for (int iSphere = 0; iSphere < nSpheres; ++iSphere)
                {
                    ...
                    BoundingSphere oldSphere = new BoundingSphere(oldCapsule.Position + MatrixHelper.GetBackward(oldCapsule.Orientation) * offset, capsuleRadius);
                    BoundingSphere newSphere = new BoundingSphere(newCapsule.Position + MatrixHelper.GetBackward(newCapsule.Orientation) * offset, capsuleRadius);
